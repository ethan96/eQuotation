Public Class USANAiAQuoteTemplate
    ...
    Public Sub setOfficeContactInfoBySales(ByVal SalesEmail As String)
    ...
    Protected Sub FillQuoteInfo(ByRef QuoteMasterRow As IBUS.iDocHeaderLine, ByRef QuoteDetailTb As IBUS.iCartList)
        ...
        If QuoteDetailTb IsNot Nothing AndAlso QuoteDetailTb.Count > 0 Then
            Dim LSSubTotal As Decimal = QuoteDetailTb.getTotalListAmount
            Dim decSubTotal As Decimal = QuoteDetailTb.getTotalAmount
            ...
            With QuoteMasterRow
                ...
                If _pri_format = EnumSetting.USPrintOutFormat.CP_Format_TOTAL_LIST_PRICE Then
                    Me.lbLPSubTotalTitle.Visible = True : Me.lbLPSubTotal.Visible = True
                End If

                Me.lbLPSubTotal.Text = _currencySign + LSSubTotal.ToString("n2")
                Me.lbUPSubTotal.Text = _currencySign + decSubTotal.ToString("n2")
                ...
                _lbExternalNote += "Send all POs to <a href='mailto:orders.aac@advantech.com'>orders.aac@advantech.com</a><br/>"
    ...
    Protected Sub gv1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
        If e.Row.RowType = DataControlRowType.Header OrElse e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If Not IsNothing(hRow) Then
                ...
                'Frank 20150702
                If _pri_format <> EnumSetting.USPrintOutFormat.CP_Format And
                    _pri_format <> EnumSetting.USPrintOutFormat.CP_Format_TOTAL_LIST_PRICE Then
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If IsBtoParentItem Then
                        ...
                        'Frank(2013 / 9 / 17)
                        Dim _ListPricTotalamount As Decimal = 0
                        Dim _UnitTotalamount As Decimal = 0

                        Dim DTL As IBUS.iCartList = DTList.Merge2NewCartList(DTList.Where(Function(X) X.parentLineNo.Value = DBITEM.Item("line_No")))
                        _ListPricTotalamount = DTL.getTotalListAmount()
                        _UnitTotalamount = DTL.getTotalAmount()
                        ...
                        'End If
                        CType(e.Row.FindControl("lbListPrice"), Label).Text = FormatNumber(_ListPricTotalamount / DBITEM.Item("qty"), 2)


                        CType(e.Row.FindControl("lbSubTotal"), Label).Text = FormatNumber(_UnitTotalamount, 2)
                        CType(e.Row.FindControl("lbUnitPrice"), Label).Text = FormatNumber(_UnitTotalamount / DBITEM.Item("qty"), 2)
                    End If
                    ...
                    Select Case _pri_format
                        ...
                        Case EnumSetting.USPrintOutFormat.CP_Format
                            If Not IsNothing(partNoCell) Then
                                partNoCell.Text = DBITEM.Item("partNo").ToString
                            End If

                        Case EnumSetting.USPrintOutFormat.CP_Format_TOTAL_LIST_PRICE
                            If Not IsNothing(partNoCell) Then
                                partNoCell.Text = DBITEM.Item("partNo").ToString
                            End If
